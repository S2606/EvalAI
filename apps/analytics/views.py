...
def get_last_submission_time(request, challenge_pk, challenge_phase_pk, submission_by):
    ...
    if submission_by == 'challenge_phase':
        last_submitted_at = Submission.objects.filter(challenge_phase=challenge_phase,
    ...
    elif submission_by == 'challenge':
        last_submitted_at = Submission.objects.filter(challenge_phase__challenge=challenge)
        last_submitted_at = last_submitted_at.order_by('-submitted_at')[0].created_at
        last_submitted_at = LastSubmissionTime(last_submitted_at)
        serializer = LastSubmissionTimeSerializer(last_submitted_at)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    # To get the last submission time by a participant team in a challenge phase.
        ...
        if not participant_team:
            response_data = {'error': 'Participant team for {} does not exist!'.format(request.user)}
            return Response(response_data, status=status.HTTP_404_NOT_FOUND)

        last_submitted_at = Submission.objects.filter(participant_team__in=participant_team,
